...
def admin_required(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            return jsonify({"Message":"Login Please"}),401
        if isAdmin == False:
            return jsonify({"messsage": "Login as an admin to access this route"}), 401
        return func(*args, **kwargs)
    return wrapper
...
def non_admin_required(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            return jsonify({"Message":"Login Please"}),401
        if isAdmin == True:
            return jsonify({"messsage": "Login as a user to can access this route"}), 401
        return func(*args, **kwargs)
    return wrapper
